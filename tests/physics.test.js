...
test('acceleration, braking and offroad penalty',()=>{const s=createRace(1800);s.rivals=[];for(let i=0;i<600;i++)stepRace(s,{gas:true},1/60);assert.ok(s.speed>58&&s.speed<60);const before=s.speed;stepRace(s,{brake:true},.05);assert.ok(s.speed<before);s.x=-9;for(let i=0;i<120;i++)stepRace(s,{gas:true},1/60);assert.ok(s.speed<=24);});
