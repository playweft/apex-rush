...
车辆使用独立的世界坐标、朝向和速度；赛道投影只用于路面、护栏和进度计算，不会自动修正玩家朝向。转向控制前轮，保留随速度变化的转向范围、输入平滑和有限抓地力。松开转向只回正前轮。停稳后按住刹车 0.4 秒可倒车脱困。护栏按车体朝向计算接触边界，按法向撞击速度减速并轻微回弹；玩家与电脑、电脑之间使用相同的低弹性等质量碰撞响应。电脑通过前方目标点主动转向，与玩家共用动力学。镜头平滑跟随车辆行进方向，保留手机倾斜补偿。已验证护栏擦撞/重撞区别、车车动量传递与能量损耗、起终点接缝碰撞及六车连续驾驶稳定性。
