...
通过 HTTPS 在手机浏览器打开，点击“启用重力转向”。iOS 的传感器授权由这次点击触发；Android 等无需授权的设备会直接等待有效方向数据。校准当前握姿后自动油门，左右倾斜设定相对赛道的目标方向，中位平滑回到赛道前方；最大目标偏角随速度从 20° 收紧到 12°，带转向阻尼，仍受抓地力及护栏保护限制。仅重力模式使用目标方向控制，键盘和触控保留原有转向，倒车使用手动转向。屏幕按钮控制刹车及氮气。“回正”重设中位。支持横竖屏坐标映射、死区、平滑、首次启用时的无数据提示，以及切后台后的暂停和重新校准。已启用后不会因为手机静止、没有新方向事件而关闭重力转向。
