...
function calibrate(){if(lastTilt===null){motionStatus('等待方向数据，请轻轻倾斜手机后再回正。');return;}neutral=lastTilt;smoothedSteer=0;motionStatus('已回正 · 左右倾斜转向 · 自动油门');}
...
function suspendMotion(){
 clearInput();
 if(gyro){lastTilt=null;neutral=null;world.resetHorizon();motionStatus('重力转向仍已开启，返回后等待重新校准。');}
...
document.addEventListener('visibilitychange',()=>{
 ...
 else{if(gyro){lastTilt=null;neutral=null;motionStatus('请保持舒适握姿，等待方向数据重新校准。');}if(sensorPending)sensorDeadline=performance.now()+4500;}
});
