...
import {NATURE_MODELS,createTerrain,buildTerrain,sceneryPlacements,addModelInstances,addSky} from './scenery.js';
export function createWorld(canvas){
 const renderer=new THREE.WebGLRenderer({canvas,antialias:true,powerPreference:'high-performance'});
 renderer.setPixelRatio(Math.min(devicePixelRatio,1.6));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;
 const scene=new THREE.Scene();scene.background=new THREE.Color('#cfddcf');scene.fog=new THREE.Fog('#cfddcf',240,850);const sky=addSky(scene);
 scene.add(new THREE.HemisphereLight(0xcce6ff,0x596145,1.8));const sun=new THREE.DirectionalLight(0xffe3b1,3);sun.position.set(-100,160,70);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-65,right:65,top:65,bottom:-65,near:1,far:350});sun.shadow.normalBias=.08;sun.shadow.bias=-.00015;scene.add(sun,sun.target);
 ...
 function ribbon(left,right,y,material,segments=1400,filter=()=>true){const positions=[],indices=[];for(let i=0;i<segments;i++){if(!filter(i))continue;const a=frame(i/segments*length),b=frame((i+1)/segments*length);const n=positions.length/3;for(const [f,offset]of[[a,left],[a,right],[b,left],[b,right]]){positions.push(f.p.x+f.right.x*offset,f.p.y+y,f.p.z+f.right.z*offset);}indices.push(n,n+1,n+2,n+1,n+3,n+2);}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));g.setIndex(indices);g.computeVertexNormals();const mesh=new THREE.Mesh(g,material);mesh.receiveShadow=true;scene.add(mesh);return mesh;}
 ...
 const sea=new THREE.Mesh(new THREE.PlaneGeometry(4000,4000),new THREE.MeshStandardMaterial({color:'#337e8e',roughness:.3,metalness:.25}));sea.rotation.x=-Math.PI/2;sea.position.y=-6;scene.add(sea);
 const terrain=createTerrain(track);buildTerrain(scene,terrain);
 ...
 const carsReady=Promise.all(['race','race-future','sedan-sports','cone'].map(name=>loader.loadAsync(`${import.meta.env.BASE_URL}models/kenney/${name}.glb`))).then(models=>{
   [player,...rivals].forEach((vehicle,i)=>{
     ...
     model.traverse(node=>{if(node.isMesh){node.material=node.material.clone();node.material.roughness=.4;node.castShadow=true;node.receiveShadow=true;}});
 ...
 const natureReady=Promise.all(NATURE_MODELS.map(name=>loader.loadAsync(`${import.meta.env.BASE_URL}models/nature/${name}.glb`))).then(models=>{
   const groups=sceneryPlacements(terrain);models.forEach((model,i)=>addModelInstances(scene,model.scene,groups[i]));
   const cliffs=Array.from({length:18},(_,i)=>{const a=i/18*Math.PI*2;return {x:Math.cos(a)*600,z:Math.sin(a)*600,y:-12,height:70+(i%4)*22,rotation:a,tint:.88};});
   addModelInstances(scene,models[4].scene,cliffs);
 });
 ...
 function draw(state,dt,idle=false,steer=0,rollTarget=0){const f=place(player,state.distance,state.lateral,state.heading??0);player.flame.visible=state.boosting;state.rivals.forEach((r,i)=>place(rivals[i],r.distance,r.lateral,r.heading??0));
